...
| ページ編集 | `page-edit.html` | 左右スワイプで全ページ編集。回転/フィルタ（タップで単一・長押しで全ページ適用）/撮り直し + 改善要望導線 + 破棄確認ダイアログ |
| 改善レポート送信 | `improvement-report.html` | 元画像と全フィルタの大プレビューを確認し、QRコードで送信先を読み取って改善レポートを送る画面 |
...
ページ編集 --[フィルタ右端の「開発元に改善要望」]--> 送信確認ダイアログ --> 改善レポート送信
