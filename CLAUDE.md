...
| カメラスキャン | `camera-scan.html` | カメラ撮影。Android は縦向き 3:4 / 横向き 4:3 のビューファインダーでキャプチャ全体に近い範囲を表示し、紙検出ガイド + 閉じる/サムネイルスタック切替 + 5秒後の報告用撮影チップ |
| 撮り直し | `camera-retake.html` | カメラスキャンの単発撮影モード。ページ編集から遷移し、1枚撮影で画像差替えと報告可否フラグ更新後に自動で戻る |
| ページ一覧 | `page-list.html` | クロップ済みページのグリッド + 名前変更 + 長押しドラッグで並べ替え・削除 + 共有 |
| ページ編集 | `page-edit.html` | 左右スワイプで全ページ編集。回転/フィルタ（タップで単一・長押しで全ページ適用）/撮り直し + 報告用撮影ページ限定のレポート導線 + 破棄確認ダイアログ |
| 改善レポート送信 | `improvement-report.html` | コメント、元画像、比較用追加写真、この撮影の画像処理デバッグ出力をQRコードで読み取った送信先へ送る画面 |
...
ページ編集 --[報告用撮影ページのフィルタ右端「開発元に報告」]--> 送信確認ダイアログ --> 改善レポート送信
改善レポート送信 --[QRコード読取成功 + 送信完了]--> ページ編集
改善レポート送信 --[未送信のまま戻る/キャンセル]--> 破棄確認ダイアログ --> ページ編集
```
...
- `attachments/` は比較元アプリ（CamScanner など）の出力画像やスクリーンショットであることがある。どの見た目になるかを読む一次資料として扱う
- `source.jpg` は自アプリ側の入力。`debug/` 配下の中間 PNG と `timings.jsonl`、および `attachments/` と並べて、どの処理が不足しているかを切り分ける
- 古いレポートには `extra-*` / `original.jpg` / `filter-*.jpg` が含まれる場合がある。その場合も比較元アプリの出力と自アプリ側出力として同じ考え方で扱う

分析の基本手順:

1. `agent-browser open http://localhost:3030/reports` で一覧を開き、コメントと現在フィルタから対象レポートを絞る
2. 詳細ページで `attachments/` のファイル名・見た目を確認し、必要ならローカル画像を並べたコンタクトシートを作って比較する
3. `source.jpg`、`debug/` の中間 PNG、`attachments/` を見比べて、白地の整形、影除去、色保持、手書き除去、細字保持などの「処理の性格」を言語化する
...
- `attachments/` の見た目は比較元アプリの内部実装そのものではなく、あくまで出力からの推定である。その前提を保って記述する
...
- 画像処理デバッグ出力: 通常撮影では中間画像や処理時間ログを保存しない。ファインダー表示から5秒後に出る「開発元に報告」チップをタップして撮影したページだけ、Debug / Release のビルド種別に関わらず文書検出・台形補正のセッション単位デバッグ成果物を出力する。各セッションには `metadata.json`、番号付きPNG、`timings.jsonl` を保存する。紙検出セッションは `input`、`analysis_rgba`、`grayscale`、`edge_support`、`colored_paper_mask`、`paper_mask`、iOS モデル有効時の `model_prob` / `model_mask` / `model_quad_overlay`、各 Canny 戦略の `blurred` / `edges` / `dilated_edges` / `contours_overlay`、`selected_quad_overlay`、`selected_quad.json` を含める。台形補正セッションは `input`、`input_corners.json`、`warped_rgba`、`output` を含める。metadata / timings には platform、anchor 有無、採用元（`opencv` / `model` / `opencv_model_agreed` など）、解析サイズ、スコア、モデル候補の有無・マスク面積比・平均確率・アンカー一致・OpenCV との agreement IoU、`paper_detection.model_inference` を含む処理時間を入れる。Android は app-specific external files の `image-processing-debug/`、iOS は `Application Support/ImageProcessingDebug/` 配下に保存する。スキャン画像は個人情報を含みうるため、端末内確認、開発者による明示取得、またはユーザーが改善レポート送信を実行した場合の同梱に限る。
...
- 紙検出: GaussianBlur → Canny(複数閾値) → dilate(3x3) → findContours(RETR_LIST) → approxPolyDP → スコアリング選択。面積最大ではなく長方形度・平行度・中央への近さのスコアで選択し、前面の紙より外側・背面の四角形が勝ちにくいようにする。iOS では PageSegNet（MobileNetV3-Small encoder + 軽量U-Net decoder、320x320 RGB → 1ch紙領域確率マップ、約1.08M params / fp16 mlprogram 約2.4MB）を紙領域の事前情報として使う。まず最良の OpenCV 候補を確定し、モデル四角形との IoU が `0.85` 以上なら OpenCV 境界を採用する。IoU が `0.35` 以上 `0.85` 未満でも、OpenCV の4辺すべてに十分な画像エッジがあり、平均値と最弱辺がモデル境界を明確に上回る場合は `opencv_edge_supported` として OpenCV 境界を採用する。それ以外はモデル候補を採用し、320px マスク由来の平均的な境界収縮を補うため重心から外向きに `2%` 拡張する。エッジ証拠だけで上書きすると背景の強い四角形を拾うため、同一対象を示す最低 IoU を必須とする。モデル候補はマスク二値化、平均確率、端接触時の面積比、アンカー一致でゲートし、ロード・推論失敗時は従来の OpenCV のみへ完全フォールバックする。Android は今回未対応。リアルタイム表示は5フレーム安定化+500ms保持
...
- Android のカメラスキャンでは `PreviewView` を全画面クロップせず、縦向きは 3:4、横向きは 4:3 のビューファインダー内に `FIT_CENTER` で表示する。横向きでは縦向きレイアウトを90度回した見え方に近づけ、操作系を右側に配置する。検出座標をオーバーレイに描画する際は、このビューファインダー枠内の letterbox を考慮して座標変換すること。
