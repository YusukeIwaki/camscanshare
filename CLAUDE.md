...
| カメラスキャン | `camera-scan.html` | カメラ撮影。Android は 3:4 のビューファインダーでキャプチャ全体に近い範囲を表示し、紙検出ガイド + 左下に閉じる/サムネイルスタック切替 + 5秒後の報告用撮影チップ |
...
- Android のカメラスキャンでは `PreviewView` を全画面クロップせず、3:4 のビューファインダー内に `FIT_CENTER` で表示する。検出座標をオーバーレイに描画する際は、この 3:4 枠内の letterbox を考慮して座標変換すること。
